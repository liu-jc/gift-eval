...
This module provides a wrapper to evaluate LLM models (GPT, Qwen, Gemini, etc.) 
on the GIFT-Eval benchmark using API calls.

Supported Providers:
    - OpenAI: GPT-4o, GPT-4-turbo, etc.
    - Qwen (DashScope): qwen-max, qwen2.5-72b-instruct, etc.
    - OpenRouter: qwen/qwen3-max, and 100+ other models
    - Anthropic: Claude models
    - Google: Gemini models

Usage:
    1. Set your API configuration in the CONFIG section below
    2. For Qwen: Set DASHSCOPE_API_KEY environment variable (optional)
    3. For OpenRouter: Set OPENROUTER_API_KEY environment variable (optional)
    4. Run the script: python llm_predictor_qwen.py
    5. Results will be saved to ../results/your_model_name/all_results.csv
"""
...
# API imports (will be imported dynamically based on api_provider)
# import openai  # For OpenAI GPT models (works with openai==0.28.0)
# import requests  # For Qwen and OpenRouter models (OpenAI-compatible API, no version conflict)
...
CONFIG = {
    ...
    # API Configuration
    "api_provider": "openrouter",  # Options: "openai", "anthropic", "google", "qwen", "openrouter"
    "api_key": "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",  # OpenRouter API key
    "model_name": "qwen/qwen3-max",  # For OpenRouter: "qwen/qwen3-max", "qwen/qwen-2.5-72b-instruct", etc.
    ...
    "enable_reasoning": True,  # If True, prompt LLM to show reasoning before prediction
    
    # Evaluation Configuration
    "batch_size": 4,  # Batch size for evaluation (reduce if API rate limits)
    "output_model_name": "qwen3-max-openrouter-thinking",  # Name to use in results file
    ...
    "max_workers": 4,  # Number of parallel threads (adjust based on API rate limits)
...
class LLMTimeSeriesPredictor:
    ...
    def __init__(
        ...
        Args:
            api_provider: API provider name ("openai", "anthropic", "google", "qwen", "openrouter")
            api_key: API key for authentication
            model_name: Model name (e.g., "gpt-4o", "qwen-max", "qwen/qwen3-max" for OpenRouter)
            ...
            base_url: Base URL for API (for qwen and openrouter providers)
        """
    ...
    def _init_api_client(self):
        ...
        if self.api_provider == "openai":
            ...
            # Qwen uses OpenAI-compatible API with requests library (no need for openai>=1.0)
            # We'll use requests directly to avoid version conflicts
            import requests
            # Get base_url from instance or environment variable
            base_url = self.base_url or os.getenv("QWEN_BASE_URL", "https://dashscope-intl.aliyuncs.com/compatible-mode/v1")
            # Get API key from instance or environment variable
            api_key = self.api_key or os.getenv("DASHSCOPE_API_KEY")
            if not api_key:
                raise RuntimeError("Qwen API key not set. Please set DASHSCOPE_API_KEY environment variable or provide api_key in CONFIG.")
            # Store config for Qwen (we'll use requests directly in _call_api)
            self.qwen_config = {
                "api_key": api_key,
                "base_url": base_url,
                "endpoint": f"{base_url}/chat/completions"
            }
            self.client = requests  # Use requests library
            self.logger.info(f"Initialized Qwen client with base_url: {base_url}")
            ...
            if not api_key:
                raise RuntimeError("OpenRouter API key not set. Please set OPENROUTER_API_KEY environment variable or provide api_key in CONFIG.")
            # Store config for OpenRouter (we'll use requests directly in _call_api)
        ...
        for attempt in range(max_retries):
            try:
                if self.api_provider == "openai":
                    ...
                    # Qwen uses OpenAI-compatible API via requests (compatible with openai 0.28.0)
                    headers = {
                    ...
                    # OpenRouter uses OpenAI-compatible API via requests
                    headers = {
                        "Authorization": f"Bearer {self.openrouter_config['api_key']}",
                        "Content-Type": "application/json",
                        "HTTP-Referer": "https://github.com/salesforce/gift-eval",  # Optional: for ranking
                        "X-Title": "GIFT-Eval Benchmark",  # Optional: for rankings
                    }
    ...
    for config_idx, (ds_name, term) in enumerate(dataset_configs_to_run, 1):
        ...
        try:
            ...
            predictor = LLMTimeSeriesPredictor(
                ...
                base_url=CONFIG.get("base_url"),  # For qwen provider
            )
